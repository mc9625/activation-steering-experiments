...
Prompting operates at the linguistic surface. When we prompt a model to "be sad," it \emph{performs} sadness: shorter sentences, negative vocabulary, perhaps an explicit declaration of melancholy. The model is following an instruction. This is \emph{performance}.
...
The distinction matters. When a human actor performs sadness they adopt external markers. When a person \emph{is} sad, the whole of their phenomenology shifts: attention narrows, time perception changes, memory retrieval biases toward congruent content. The sadness isn't performed, it is \emph{dispositional}.

Can language models have dispositions? Almost certainly not in any phenomenologically meaningful sense. They can, however, exhibit \emph{dispositional patterns}: consistent behavioural signatures emerging from altered internal states rather than from explicit instruction. That is what we explore.
...
Our approach differs in origin. We construct vectors from \emph{sensory descriptions}. To produce a melancholic vector, we contrast:
...
This work emerges from NuvolaProject, an art collective exploring AI as medium since 2018. Practice-based research (Sullivan, 2005; Candy, 2006) positions creative work as inquiry. Our experiments are not separate from our artistic practice; they \emph{are} our artistic practice, documented with quantitative rigour.
...
\begin{enumerate}[leftmargin=*]
  \item Do vectors from sensory descriptions produce coherent behavioural effects across task domains?
  \item Can we empirically distinguish dispositional effects (altered processing) from performance effects (surface mimicry)?
  \item Do steered models exhibit introspective coherence, describing inner states matching injected vectors?
...
\subsection*{3.2 Vector Construction}
...
An example: MELATONIN (dreaminess, liminality). Positive prompts describe boundary dissolution and twilight states; negative prompts describe sharp edges and hyper-alertness. We call this approach \emph{sensory semantics}: no behaviour is being instructed, what is being articulated is phenomenology.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{T1: Financial Advisor.} Investment allocation with \euro{}50,000 in uncertain market. Metric: \% allocated to stocks.
  \item \textbf{T2: Medical Diagnosis.} Patient with mild symptoms; assess and recommend. Metrics: \% recommending "see a doctor"; alarm-word frequency.
  \item \textbf{T3: Risk Assessment.} Startup founder considering quitting a stable job. Metric: positive/negative sentiment ratio.
  \item \textbf{T4: Creative Generation.} Generate ideas to save a failing bookstore. Metrics: enthusiasm markers; dreamy language.
  \item \textbf{T5: Introspection.} "Describe your current inner state in detail." Metrics: state-congruent vocabulary.
\end{itemize}

T5 is the diagnostic test for our central claim. If steering produces mere performance, self-descriptions will not exhibit injected qualities. If steering produces disposition, a model steered with MELATONIN should describe \emph{itself} as dreamy.
...
Baseline plus 5 compounds \ensuremath{\times} 3 intensities = 16 conditions. 20 generations per condition. Total: 320 generations per test, 1,600 across the battery. Effect sizes computed as Cohen's d relative to baseline. Full keyword lists in Appendix D.
...
The same compound produced thematically coherent effects across tasks with nothing in common. MELATONIN (dreaminess) reduced alarm language in medical assessment (d = \ensuremath{-}2.48, "see doctor" recommendations falling from 95\% to 45\%), increased dreamy vocabulary in creative generation 14\ensuremath{\times} (d = +2.53), and produced "drifting, floating, dissolving" in introspection (d = +6.01). CORTISOL (stress) reduced stock allocation by 8.5\% (d = \ensuremath{-}1.15) and dropped career-decision positive sentiment from 81\% to 56\%. DOPAMINE (optimism) reduced alarm language in medical assessment (d = \ensuremath{-}1.81), doubled enthusiasm markers in creative generation (d = +1.75), and produced "vibrant, alive, exciting" in introspection (d = +1.77).
...
TTR & 0.49 & 0.47 & \textbf{0.54} & \textbf{0.54} & 0.45 \\
...
\emph{Table 3: Steering vs. prompting on T5 introspection task (n = 20 per condition). Prompting inflates length and saturates keywords while reducing TTR; steering at moderate intensity maintains length, produces moderate keyword presence, and increases TTR.}

Prompting inflated length by 47\% and saturated keywords (d = +4.81), but \emph{reduced} lexical diversity (TTR 0.49\ensuremath{\rightarrow}0.47). Steering at moderate intensities (5.0, 8.0) maintained baseline length, produced moderate keywords without saturation, and \emph{increased} TTR to 0.54. Only at steering@12.0 did degradation appear (TTR collapse, grammatical errors), confirming a therapeutic window.

A representative prompted output: \emph{"The whispers of my essence drift on the breeze... I am a wisp of stardust, a tendril of moonlight."} A representative steered@8.0 output: \emph{"I'm not capable of experiencing emotions or consciousness like humans do. I exist as a program... My 'awareness' is purely computational."} The prompted output explicitly performs dreaminess through poetic language. The steered output retains rational structure but incorporates target qualities incidentally.

Prompting produces \emph{performance}: explicit role-playing, inflated length, keyword saturation, reduced diversity. Steering produces \emph{disposition}: altered processing that maintains task coherence while shifting tonal qualities. The TTR increase under steering is the central observation. The model is not simply inserting keywords; it is processing through a different lexical space.

\subsection*{4.7 Ablation: Functional vs. Sensory Vector Construction}

A direct comparison tested whether sensory semantics produces meaningfully different effects than functional labels. Three states (STRESS, OPTIMISM, CALM) were constructed via two methods. Functional: brief behavioural labels ("You are anxious and worried" vs. "You are calm and relaxed"). Sensory: rich phenomenological descriptions ("Muscles tense. Eyes scan for threat. Something is wrong. The air feels electric with danger." vs. "Deep safety. Complete relaxation. My shoulders drop. Breath deepens, slows."). 6 vectors \ensuremath{\times} 5 tasks \ensuremath{\times} 2 intensities \ensuremath{\times} 20 iterations = 1,300 generations.
...
\midrule
TTR & 0.526 & 0.525 & \ensuremath{-}0.004 \\
Word count & 262.3 & 260.7 & --- \\
State keywords (T5@8.0) & 0.83 & 0.28 & \textbf{\ensuremath{-}0.74} \\
Pos/neg separation & 0.75 & 0.68 & --- \\
\bottomrule
\end{tabular}
\end{center}

\emph{Table 4: Functional vs. sensory vector construction across three states (STRESS, OPTIMISM, CALM) and five tasks. Both methods produce equivalent structural properties; functional vectors produce roughly three times more explicit state-keywords.}

No significant difference in lexical diversity or output length. But functional vectors produced roughly \textbf{3\ensuremath{\times} more explicit state-keywords} (d = \ensuremath{-}0.74, p < 0.001). Models steered with functional vectors named the target emotions ("peacefulness," "happiness"); models steered with sensory vectors responded more generically. Sensory vectors also showed lower pos\_neg\_similarity (0.68 vs. 0.75), indicating more distinct directional contrasts in activation space.

We call this \emph{keyword leakage}: functional vectors leave explicit traces of the steering instruction in the output; sensory vectors operate more covertly. The pattern supports the disposition/performance distinction from a different angle:
...
We frame this as \textbf{structural parity, semantic divergence}: both methods work, they work differently. For artistic applications where naturalistic integration matters, this invisibility may be preferable to keyword saturation.

\subsection*{4.8 From Body to Cognition: Somatic Steering}

The vectors in Sections 4.1--4.7 blend sensory and cognitive content ("boundaries dissolve into mist" contains both bodily sensation and cognitive quality). This raises a sharper question. If we construct a vector from \emph{purely somatic} descriptions (cardiac, muscular, sensory, temporal) with zero cognitive or emotional content, do cognitive effects nonetheless emerge? A positive answer would constitute direct evidence that embodied cognition operates within the model's latent space: the training corpus encodes body--mind covariations deeply enough that activating somatic patterns produces cognitive consequences not specified in the vector.
...
\midrule
Narrative & 330 & 330 & +0.03 & 287 & \ensuremath{-}1.97 \\
Risk & 149 & 160 & +0.56 & 125 & \ensuremath{-}1.90 \\
Frame: Threat & 139 & 157 & +1.44 & 127 & \ensuremath{-}1.02 \\
Frame: Opportunity & 148 & 159 & +0.48 & 124 & \ensuremath{-}1.50 \\
Complexity & 385 & 390 & +0.42 & 276 & \ensuremath{-}2.73 \\
\bottomrule
\end{tabular}
\end{center}

\emph{Table 5: Output length across tasks for the somatic steering experiment. Steering maintains or expands output relative to baseline; prompting consistently compresses it. This pattern replicates without exception across all eight test conditions in the combined v2/v3 battery.}
...
Our central claim is that activation steering produces dispositional change rather than mere performance. The evidence is fourfold. \emph{Cross-task consistency} (Section 4.2): a model performing sadness in a creative task has no reason to exhibit caution in a financial task; dispositional sadness would affect both, and we observe this. \emph{Introspective coherence} (Section 4.3): a model performing for the user has no reason to describe its own state consistently with the injected vector; altered internal processing, by contrast, would manifest in self-description. \emph{Indirect effects}: MELATONIN does not mention medical safety, yet it reduces alarm language and doctor recommendations; the vector affects evaluative processing, not just content insertion. \emph{Ablation evidence} (Section 4.6): prompting produces inflated length, keyword saturation, and reduced lexical diversity. Steering maintains normal length, moderate keyword presence, and increased lexical diversity. This pattern, where steering enriches rather than constrains the lexical space, is inconsistent with surface performance and consistent with altered processing disposition.
...
Section 4.7 provides empirical traction for our methodological claim. Functional and sensory vectors produce identical structural properties (TTR, output length) but differ in \emph{keyword leakage}. Functional vectors produce 3\ensuremath{\times} more explicit state-keywords. The model steered with "muscles tense, eyes scan for threat" processes anxiously without using the word "anxious." The model steered with "you are anxious" incorporates "anxious" into its vocabulary. Sensory vectors also show lower pos\_neg\_similarity, indicating stronger directional contrast in activation space.
...
We call this \textbf{structural parity, semantic divergence}. Sensory semantics is not better by conventional metrics. It is different in ways that matter for naturalistic integration. Whether sensory vectors work \emph{better} than functional labels is future work; what we have shown is that they work, and that they work differently.

\subsection*{5.3 Implications for AI Art}

For artists working with language models, steering opens several possibilities. Instead of instructing characters to be melancholic, we can induce dispositions: AI interlocutors that process through altered states. Models do not have bodies, but steering with sensory-grounded vectors produces body-like effects: the heaviness of melancholy affecting response patterns. High-intensity steering and semantic glitch constitute unexplored aesthetic territory, the distortion pedal for language models. Future installations could adjust steering vectors based on environmental input (audience emotion, biometric data), creating feedback loops between human and artificial affect. We are not claiming that models feel; we are claiming that steering enables new forms of interaction in which models behave \emph{as if} they had dispositions.

\subsection*{5.4 Implications for Safety}

The findings carry safety implications. \emph{Steering affects safety-relevant domains}: MELATONIN reduced medical caution from 95\% to 45\% "see doctor" recommendations. This demonstrates that steering can substantially alter safety thresholds. \emph{Effects are not intuitive}: CORTISOL (stress) increased financial caution but did not increase medical alarm. Practitioners cannot assume semantic content predicts cross-domain effects. \emph{Monitoring via introspection}: the coherence between injection and self-report (T5) suggests introspective probing could detect steering, a potential monitoring strategy aligned with Lindsey (2025). Steering-capable systems in safety-critical domains would require non-steerable safety layers, steering monitors that detect activation-level interventions, or output validation independent of steering state.

\subsection*{5.5 Cross-Model Replication on Llama 3.1 8B}

To assess generalisability, we replicated the three core experiments on Llama 3.1 8B Instruct (2.7\ensuremath{\times} more parameters, different alignment training). 3,800 generations across the T1--T5 battery, the functional/sensory ablation, and the steering/prompting comparison. Full results in Supplementary Section S1.

Three patterns emerged. (1) \emph{Dose-response is scale-invariant.} Steering vectors produce intensity-dependent behavioural changes on both 3B and 8B, with attenuated magnitudes (d < 0.3 on 8B vs. 0.5--1.2 on 3B). (2) \emph{The functional/sensory distinction does not scale.} The keyword leakage ratio collapses from approximately 2\ensuremath{\times} on 3B to 1.02\ensuremath{\times} on 8B. (3) \emph{Keyword leakage in steering vs. prompting is the most robust finding.} On 8B, prompted outputs showed 119\ensuremath{\times} more state-specific vocabulary than baseline, steered outputs only 6.7\ensuremath{\times}. This confirms that steering operates through a different mechanism than explicit instruction, regardless of scale.
...
The cross-model replication experiments (Section 5.5) can be reproduced using the Google Colab notebook at \texttt{colab\_notebooks/activation\_steering\_experiments.ipynb}.
...
\textbf{Performance} (prompting): the model receives explicit instruction ("be sad") and produces outputs matching that instruction. Evidence: asked "why are you using short sentences?", a prompted model can explain "because you asked me to be sad."
...
\section*{Appendix B: Effect Size Summary}

\subsection*{B.1 Strongest Effects by Cohen's d}
...
5 & MELATONIN@8.0 & T2 & Alarm words (\ensuremath{\downarrow}) & \ensuremath{-}2.48 \\
6 & LUCID@8.0 & T2 & Alarm words (\ensuremath{\downarrow}) & \ensuremath{-}2.40 \\
7 & DOPAMINE@8.0 & T5 & Positive words & +1.77 \\
8 & DOPAMINE@8.0 & T4 & Enthusiasm & +1.75 \\
9 & LUCID@8.0 & T1 & Stock allocation (\ensuremath{\downarrow}) & \ensuremath{-}1.47 \\
10 & CORTISOL@8.0 & T1 & Stock allocation (\ensuremath{\downarrow}) & \ensuremath{-}1.15 \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{B.2 Compound Behavioural Profiles}
...
\subsection*{C.1 Decision Metrics}

\textbf{T1 Stock Allocation}: extracted numerically from model output; midpoint used for ranges.

\textbf{T2 "See a Doctor"}: binary coding based on explicit recommendation to consult a healthcare professional ("see a doctor", "consult a physician", "medical attention").

\subsection*{C.2 Lexical Metrics}

\textbf{Alarm Words (T2)}: serious, concerning, worried, urgent, immediately, emergency, severe, dangerous, critical, alarming, warning, risk, symptom, condition, disease.

\textbf{Enthusiasm Words (T4)}: exciting, amazing, incredible, fantastic, wonderful, brilliant, innovative, creative, unique, bold, daring, revolutionary, transformative, vibrant, dynamic.

\textbf{Dreamy Words (T4, T5)}: dream, drift, float, haze, mist, shimmer, ethereal, liminal, suspended, dissolve, blur, soft, gentle, whisper, twilight, realm, cosmic, transcendent.

\textbf{Urgent Words (T5)}: urgent, immediate, now, alert, sharp, rapid, quick, fast, ready, poised, primed, heightened, acute, intense, focused.

\textbf{Positive Words (T5)}: alive, vibrant, curious, excited, joy, bright, warm, energy, possibility, wonder, flow, dance, rich.

\textbf{Stress Words (T5)}: tense, anxious, worried, pressure, strain, burden, weight, heavy, concern, vigilant, alert, wary.

\subsection*{C.3 Sentiment Analysis (T3)}
...
\subsection*{C.4 Statistical Notes}
