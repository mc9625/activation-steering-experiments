...
\begin{enumerate}
    ...
    \item \textbf{Ablation evidence}: Direct comparison (Section 4.4) shows prompting produces inflated length (+47\%), keyword saturation, and \textit{reduced} lexical diversity. Steering maintains normal length, moderate keywords, and \textit{increased} diversity---inconsistent with surface performance, consistent with altered processing disposition
\end{enumerate}
...
Single model (Llama 3.2 3B); single prompt per task; no human evaluation. The steering vs prompting ablation (Section 4.4) tested only MELATONIN on T5; systematic comparison across all compounds and tasks remains future work.
...
\textbf{Introspection task}: The model's semantic knowledge of target vocabulary (e.g., ``dreamy,'' ``urgent'') may contaminate introspective responses. However, the ablation study partially addresses this: if semantic knowledge alone drove effects, prompting and steering should produce similar patterns. They do not---prompting reduces lexical diversity while steering increases it, suggesting different underlying mechanisms.
