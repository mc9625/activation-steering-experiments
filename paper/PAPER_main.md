...
The steering effect generalises to state-congruent vocabulary the model never saw during vector construction: more than half of steered generations at α = 8.0 contain at least one held-out dreamy word, against zero at baseline. The effect is weaker and more diffuse than under prompting, which is consistent with the disposition framing: a shift of the sampling distribution toward a semantic region, rather than a saturated performance of it. This rules out the strictest deflationary reading (pure construction-token regurgitation); the weaker reading, that steering biases a broad semantic neighbourhood, survives, but it is not deflationary: a distributed shift of the sampling distribution toward a semantic region is our operational definition of disposition.
