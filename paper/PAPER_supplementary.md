...
¹ NuvolaProject, Rome, Italy · massimo@nuvolaproject.cloud
...
*Table S1: Cross-model comparison at intensity 8.0. 8B shows lower baseline TTR and substantially reduced keyword presence.*
...
2. *Effect sizes attenuated.* Cohen's d values ranged from 0.06 to 0.31 on 8B versus 0.5–1.2 on 3B. The steering mechanism operates, but with reduced magnitude.
...
On Llama 3.2 3B, functional vectors produced approximately 2× more state-specific keywords than sensory vectors. On 8B, this ratio collapsed to 1.02×. Effect size for the functional/sensory distinction: d = 0.036 (negligible).
...
*Table S3: Steering vs. prompting on Llama 3.1 8B.*

**Keyword leakage replicates.** Prompted outputs showed 119× more state-specific vocabulary than baseline (3.57 vs. 0.03); steered outputs only 6.7× (0.20 vs. 0.03). This confirms that steering operates through a different mechanism than explicit instruction, regardless of scale.

**TTR pattern inverts.** On 3B, prompting decreased TTR while steering increased it. On 8B, prompting *increased* TTR (+0.021) while steering *decreased* it (−0.017). Qualitative inspection reveals why. Prompted outputs on 8B become theatrically elaborated:
...
*Table S5: Directional divergences between steering and prompting.*
...
**Model specificity.** All findings specific to Gemma 4 E2B at layer 20.
...
3. The residual between multimodal and text-only processing has a **dominant scalar structure**: a well-defined divergence angle $\theta$ that varies systematically across images and correlates with perceivable image properties.
